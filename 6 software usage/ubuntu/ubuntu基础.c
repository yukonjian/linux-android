...
udp :server:./iperf -s -u -P 0 -i 1 -p 5001
     client:./iperf -u -c 172.25.2.88 -P 1 -i 1 -p 5001 -n 500M -b 10M
...
-l :length of buffer in bytes to read or write (Defaults: TCP=128K, v4 UDP=1470, v6 UDP=1450)

./iperf -s  -P 0 -i 1 -p 5001
./iperf -c 172.25.2.86 -P 1 -i 1 -p 5001 -n 500M -b 20M
...
/* 命令 */
服务端：iperf -s -o iperf.log -f M -u -i 10
            -s: iperf 运行在服务端模式
            -o: 生成的iperf.log文件
            -f: 以M（兆）得出报告
            -u: 接收udp包
    ...
    客户端：iperf  -c 172.25.2.83 -u -b 85M -f  M  -t 30000 -i 10
            -c: 标记自己为客户端
            -u: 使用传输协议为udp
            -b：指定发送带宽大小为100M
            -f：表示以M显示报告
